...
func (h *Handler) handleGetBlockchainInfo(params []interface{}) (interface{}, error) {
	// Create multiple response channels for different pieces of info
	heightCh := make(chan types.Response)
	lastBlockCh := make(chan types.Response)
	peersCh := make(chan types.Response)

	// Request height
	h.messageBus.Publish(types.Message{
		Type:       types.GetBlockchainInfo, // You might need to create a specific message type for this
		Data:       "height",
		ResponseCh: heightCh,
	})
	...
	info := struct {
		...
		Height:      0,
		ChainID:     "thrylos-testnet", // Default value
		NodeCount:   0,
		NodeVersion: "1.0.0",
		IsSyncing:   false,
	}

	// Fill in the height if available
	if heightResp.Error == nil {
		switch heightData := heightResp.Data.(type) {
		case int32:
			info.Height = heightData
		case int:
			info.Height = int32(heightData)
		case int64:
			info.Height = int32(heightData)
		case float64:
			info.Height = int32(heightData)
		default:
			log.Printf("Unexpected height data type: %T", heightResp.Data)
		}
	}

	// Fill in the last block if available
	if lastBlockResp.Error == nil {
		switch lastBlockData := lastBlockResp.Data.(type) {
		case *types.Block:
			if lastBlockData != nil {
				// Use the String() method on the *Hash type (note the pointer receiver)
				hashPtr := &lastBlockData.Hash
				info.LastBlock = hashPtr.String()
			}
		case map[string]interface{}:
			// Handle case where data is a map
			if hashData, ok := lastBlockData["hash"]; ok {
				if hashStr, ok := hashData.(string); ok {
					info.LastBlock = hashStr
				} else if hashBytes, ok := hashData.([]byte); ok {
					// Convert bytes to hex string
					info.LastBlock = hex.EncodeToString(hashBytes)
				}
			}
		default:
			log.Printf("Unexpected last block data type: %T", lastBlockResp.Data)
		}
	}

	// Fill in the peer count if available
	if peersResp.Error == nil {
		switch peersData := peersResp.Data.(type) {
		case []string:
			info.NodeCount = len(peersData)
		case []interface{}:
			info.NodeCount = len(peersData)
		default:
			log.Printf("Unexpected peers data type: %T", peersResp.Data)
		}
	}
