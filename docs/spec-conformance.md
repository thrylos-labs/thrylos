...
| A node killed at any instant restarts without operator action | **Open decision** | Tests kill real `chain-node` and `chain-signer` processes and start them again (`crates/node/tests/devnet_processes.rs`), but only at moments when the node is idle. Killing every process of a running four-node network at a random height left at least one node unable to restart in 4 of 16 trials ("refusing to sign at height 4, round 0, prevote", and at propose and precommit positions): the signer had recorded a signature the node died before recording, so on restart the node must sign that position again and the signer refuses it, as the spec requires ("unconditionally"). | Decide whether the signer may return its earlier signature, and only that one, when asked to sign the byte-identical message at exactly its mark (BLS signatures are deterministic, so it cannot be made to say two things), amend "Keys, signing and slashing safety" if so, then implement it and test a kill at every step of the signing path. |
