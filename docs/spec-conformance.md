...
| A network of one validator runs | **Conforms** | A one-validator genesis is valid (`chain-genesis check` warns about it, as advice). A host with no one to wait for commits one block per pace and hands control back (`a_validator_with_no_one_to_wait_for_commits_one_block_a_pace...`), and a real one-validator network makes a block a second and stops at the height it is asked to (`a_network_of_one_validator_makes_a_block_a_second...`). Before the pace existed such a node committed about eighteen heights a second inside one call, reported nothing and could not be stopped. | Keep the pace above zero: `tuning.block_interval_ms` and `devnet init --block-interval-ms` refuse zero for that reason. |
| Block time is a 1s target with a 2s timeout, the timeout being over 3x observed p99 propagation | **Partial** | `HostConfig::min_block_interval_ms` (default 1000; `tuning.block_interval_ms` in a node's configuration): after a commit every node waits that long before it starts the next height, so the propose timeout counts from the start of the height and keeps its two seconds (`HostConfig::timeouts`; the prevote and precommit steps use Malachite's one-second defaults). Tested in virtual time (blocks one to two seconds apart, every height decided in round 0, a paced node cut off for many heights catches up, a paced network asks no one for sync) and with real processes. | Nothing has measured propagation, so "3x p99" is unchecked. A proposal goes in full to every peer: a 4 MiB block to 127 peers is about 500 MB from the proposer, which cannot arrive in two seconds on a plausible uplink, so at a large validator set the rule needs block dissemination that is not a full mesh, or a smaller block cap. The pace is not a validity rule (a block proposed sooner is accepted), and it is measured from a node's own commit and not the parent's timestamp, so validators whose clocks differ by seconds do not drift apart in it. |
...
| Signed weak-subjectivity checkpoints constrain initial sync | **Missing** | No checkpoint release or node enforcement path (`block` gives a height and its state root, which is what a checkpoint states). | Define checkpoint format, signer policy, two publication channels and loud override semantics. |
...
| Mempool admission, replacement and eviction rules | **Partial** | Pool rules are tested, and the pool now drops what a commit made stale: executed transactions, ones its sender can no longer pay for, and expired ones (`Mempool::prune_after_commit`). In the node (`chain-node`'s `txpool`) it reads accounts and the base fee from the chain the host drives, is the host's source of transactions and the event loop's intake, and passes what is new on to peers. Four running nodes included, ran once and agreed on a transaction handed to one of them, and on five from three senders, each sender's in order. | Nothing tells a sender why a transaction was refused, and nothing outside a test can submit one until there is an RPC. The pool re-checks only the senders a block touched (and expiry for all), so a transaction made unaffordable by a balance change from anything else stays until it fails at proposal, where the host drops it. Relaying tells every peer, which is quadratic in the network, and has no rate limit beyond the transport's. |
| Minimal JSON-RPC and tracing | **Partial** | Five calls, and only these, over JSON-RPC 2.0 on a small HTTP server that is loopback-only (the crate's tier says "local only"; configuring a wider address is refused) and bounded (fixed workers, a bounded backlog that answers 503, header, body and time limits, a bounded queue to the node, all node access on the event loop's own thread): `status` (chain, head, peers, pool, why the host halted), `block` (with its state root), `commit` (the certificate), `account` (balance and next sequence number) and `send_transaction` (a refusal names the mempool rule). Addresses are `thry1…` (D-010). Tested at the protocol, server and node layers, with four real nodes over HTTP (a transaction sent to one, seen included and agreed on at another, every refusal over the wire) and with the real `chain-node devnet bump` against running processes. | No transaction index or receipts, so a client can see that its transaction was included but not that it failed in execution. No subscriptions, batches, authentication or TLS (a proxy in front is the operator's, and the server refuses a public address itself). No tracing. Nothing yet asks what the first applications need beyond these five. |
...
| Halt detection and certificate-based recovery runbook is implemented and rehearsed | **Missing** | The prose runbook exists only in the specification. The RPC now exposes what it needs to be built on: `status` says whether the host has halted and why, and where the chain is; `commit` is a node's last commit certificate, which agreement on defines the recovery point. | Add monitoring, release/restart tooling and a chaos-testnet rehearsal. |
...
2. drive the consensus host, bounded mempool and block catch-up through
   ...
   validator its peer is; the mempool hand-off is done, and the filter is
...
Observer mode, discovery, snapshots, warp sync and pruning remain outside
this gate, and the RPC is the five calls in the ledger.
