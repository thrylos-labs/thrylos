...
the chain. Listed in the order they were met, with what would fix it. **Fixed the same day: 1, 2, 4 and 7**
(marked below); 3, 5 and 6 are open or by design.

1. **(Fixed: `publish` builds a package directory first when it has no build or a source is newer.) `move publish` needed `move build` first.** The error says so, but it is a step nobody wants to
   remember. *Fix:* `publish` builds when the package is not built or its sources are newer.
2. **(Fixed: the command declares every `address:` argument.) A transfer named the recipient twice**: as an argument and again as `--input`. Forgetting the
...
4. **(Fixed with 2.) Views of another address needed `--input` too**, for the same reason as 2, and the error for
...
7. **(Fixed: an empty slot now says which slots the owner does hold that type in.) `move resource` reads slot 0 unless told otherwise**, and when the drawer is empty the answer
